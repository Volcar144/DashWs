...
def load_or_create_config():
    ...
    defaults = {
        ...
        'auth': {
            'jwt_secret': 'super-secret-key-' + os.urandom(16).hex(),
        ...
        'api_endpoints': {
            'weather': {
                ...
                'endpoints': {
                    'current': 'current?location=London',
                    'forecast': 'forecast?days=3'
                },
                'api_key': 'YOUR_WEATHER_API_KEY'
            }
        },
        'rate_limits': {
            'weather': {'max': 10, 'replenish': 3}
        },
        'request_threading': {
            'max_workers': 10,
            'min_workers': 2,
...
def configure_logging(config):
    ...
    # Create rotating log handler with UTF-8 encoding
    file_handler = RotatingFileHandler(
        log_file,
        maxBytes=config['logging']['max_bytes'],
        backupCount=config['logging']['backup_count'],
        encoding='utf-8'  # Fix for Unicode errors
    )
